# -*- coding: utf-8 -*-
"""WORKING SINGLE COLORED BEDROCK  [DS' UNET] Images.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/16IIqNx67NsK-C7H1BWk93CE2dxg6I0EN
...
def change_distribution(img):
  ...
  BROWN_SOIL = [255, 255, 255] #[0] 
  BLUE_BR = [1, 1, 1] #[0, 0, 255]
  YELLOW_SAND = [255, 255, 255] #[255, 255, 0]
  RED_BIGR = [255, 255, 255] #[255, 0, 0]
  WHITE = [255, 255, 255]
...
def get_bool_distribution(img):
  ...
  print("Bedrock BLUE [1]: " + str(zero_soil) + " pixels")
  print("None [255]: " + str(white) + " pixels")
  print("Total Pixels: " + str(pixel_ttoal) + " pixels")
...
for n, id_ in tqdm(enumerate(test_ids), total=len(test_ids)):   
    path = VALIDATION_DATA_MASKS + id_
    print(path + '.png')
    img = cv2.imread(path + '.png')[:,:, :IMG_CHANNELS]
    img = cv2.resize(img, (IMG_HEIGHT, IMG_WIDTH), interpolation = cv2.INTER_NEAREST)
    Y_test[n] = img
...
"""/content/drive/MyDrive/Energy Analytics/ai4mars-dataset-merged-0.1/DATASET/validation_masks/NLB_565197947EDR_F0670490NCAM00256M1.png"""
...
plt.show()
img = change_distribution(original_Y[image_x])
imshow(np.squeeze(img))
plt.show()

img

get_distribution(img)

for i, img in tqdm(enumerate(Y_train), total=len(X_train)):   
  Y_train[i] = change_mask_to_bool(img)
  print(Y_train[i])
